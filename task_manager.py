...
def view_mine(list_of_tasks, current_user):
...
def edit_choice(number_of_task, task_number_dictionary, master_task_list):
    ...
    if task['completed']:
        print("This task has already been completed, and therefore cannot be edited.")
        return None, None, master_task_list
...
2 - Edit the task\n''')
    ...
    if 1 <= option_selection <= 2:
        ...
        return None, edit_choice(number_of_task, task_number_dictionary, 
...
: ''').lower()
    ...
    # Calls the function to view the current user's tasks in a user-friendly way
        ...
        view_my_tasks, tasks_number_index = view_mine(task_list, curr_user)     
